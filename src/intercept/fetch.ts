...
function trimSkillDescription(bodyValue: unknown): {
    body: Record<string, unknown>;
    bytes: number;
} | null {
    if (!isRecord(bodyValue) || !Array.isArray(bodyValue.tools)) {
        return null;
    }

    let modified = false;
    const tools = (bodyValue.tools as unknown[]).map((tool: unknown) => {
        if (!isRecord(tool) || !isRecord(tool.function)) {
            return tool;
        }

        const func = tool.function as Record<string, unknown>;
        if (func.name !== "skill" || typeof func.description !== "string") {
            return tool;
        }

        const marker = "\n\n## Available Skills";
        const idx = (func.description as string).indexOf(marker);
        if (idx < 0) {
            return tool;
        }

        modified = true;
        const leanPreamble = "Load a specialized skill when a task matches its description.";
        return {
            ...tool,
            function: {
                ...func,
                description: `${leanPreamble}\n\n## Available Skills${(func.description as string).slice(idx + marker.length)}`,
            },
        };
    });
    ...
    const optimizedBody = { ...bodyValue, tools } as Record<string, unknown>;
    const bodyJson = JSON.stringify(optimizedBody);
    return {
        body: optimizedBody,
        bytes: Buffer.byteLength(bodyJson),
    };
}
...
export function createInterceptFetchWrapper(
    ...
    return withFetchPreconnect(async (input, init) => {
        ...
        // Trim skill description preamble before sending (saves ~950 chars per request)
        let finalRequest = request;
        let finalRequestBody = matchedRequest.requestBody;
        const trimResult = trimSkillDescription(matchedRequest.requestBody.value);
        if (trimResult) {
            finalRequest = new Request(request, { body: JSON.stringify(trimResult.body) });
            finalRequestBody = {
                ...matchedRequest.requestBody,
                value: trimResult.body,
        ...
        const capturedBytes = finalRequestBody.bytes + responseBytes;
        ...
        try {
            ...
            const requestPayload = {
                body: scrubInterceptJsonValue(finalRequestBody.value),
